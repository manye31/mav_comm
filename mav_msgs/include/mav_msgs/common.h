...
inline bool isRotationMatrix(const Eigen::Matrix3d& mat){
  ...
  if ((mat.transpose() * mat - Eigen::Matrix3d::Identity()).norm() > kSmallValueCheck){
    std::cerr << "[mav_msgs::msg::common] Rotation matrix requirement violated (R^T * R = I)" << std::endl;
    return false;
  }  
  // Check that det(R) = 1
  if (mat.determinant() - 1.0 > kSmallValueCheck){
    std::cerr << "[mav_msgs::msg::common] Rotation matrix requirement violated (det(R) = 1)" << std::endl;
    return false;
  }  
  return true;
}
...
inline bool vectorFromRotationMatrix(const Eigen::Matrix3d& mat,
  ...
  if (!isRotationMatrix(mat)){
    std::cerr << "[mav_msgs::msg::common] Not a rotation matrix." << std::endl;
    return false;
  }
